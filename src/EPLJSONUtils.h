...
public:
	...
	//! Get string by name
	static std::string GetStringByName(const nlohmann::json& inJSON, const std::string& inName, const std::string& defaultValue = "")
	{
		// Check desired value exists
		nlohmann::json::const_iterator iter(inJSON.find(inName));
		if (iter == inJSON.end())
	...
	static unsigned int GetUnsignedIntByName(const nlohmann::json& inJSON, const std::string& inName, unsigned int defaultValue = 0)
	{
		// Check desired value exists
		nlohmann::json::const_iterator iter(inJSON.find(inName));
		if (iter == inJSON.end())
